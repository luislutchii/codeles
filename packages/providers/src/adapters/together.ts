/**
 * CodeLES Together AI Provider Adapter
 * Adapter para Together AI API
 */
...
export class TogetherAdapter extends BaseProviderAdapter {
  ...
  async initialize(config: ProviderConfig): Promise<void> {
    ...
    if (!validation.valid) {
      throw new Error(`Together validation failed: ${validation.errors.join(', ')}`);
    }
  ...
  async chat(request: ProviderRequest): Promise<ProviderResponse> {
    if (!this.client) throw new Error('Together client not initialized');
  ...
  async listModels(): Promise<ModelConfig[]> {
    return [
      ...
      { id: 'meta-llama/Meta-Llama-3.1-8B-Instruct-Turbo', name: 'Llama 3.1 8B', contextWindow: 128000, maxOutputTokens: 4096, supportsStreaming: true, supportsTools: true, supportsVision: false, supportsJsonMode: true, capabilities: ['chat', 'tools', 'json_mode', 'streaming'] },
      { id: 'mistralai/Mixtral-8x7B-Instruct-v0.1', name: 'Mixtral 8x7B', contextWindow: 32768, maxOutputTokens: 4096, supportsStreaming: true, supportsTools: true, supportsVision: false, supportsJsonMode: true, capabilities: ['chat', 'tools', 'json_mode', 'streaming'] },
      { id: 'Qwen/Qwen2-72B-Instruct', name: 'Qwen2 72B', contextWindow: 32768, maxOutputTokens: 4096, supportsStreaming: true, supportsTools: true, supportsVision: false, supportsJsonMode: true, capabilities: ['chat', 'tools', 'json_mode', 'streaming'] }
    ];
  }
